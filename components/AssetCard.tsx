...
export function AssetCard({ 
  ...
  const displayCategory = asset.category ? (categoryMap[asset.category.toLowerCase()] || asset.category) : '未分类';
      ...
      >
        ...
        <div className="absolute top-4 left-4 right-4 z-10 flex justify-between items-start">
          <div className="flex flex-col">
            <span className="text-[8px] font-bold uppercase tracking-widest text-white/40">{displayCategory}</span>
          </div>
          ...
          <div className="flex items-center justify-between">
            <div className="flex gap-1">
              {asset.tags.slice(0, 2).map(tag => (
                <span key={tag} className="text-[7px] font-bold uppercase tracking-widest text-white/40 border border-white/10 px-1.5 py-0.5 rounded-full">
                  {tag}
                </span>
              ))}
            </div>
              ...
              <div className="w-full lg:w-[450px] p-12 lg:p-16 flex flex-col justify-between bg-black/40">
                <div className="space-y-12">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {asset.tags.map(tag => (
                        <span key={tag} className="px-4 py-2 rounded-full border border-white/5 text-[10px] font-bold uppercase tracking-widest text-white/20">
                          {tag}
                        </span>
                      ))}
                    </div>
                ...
                <div className="space-y-6">
                  <div className="flex items-center gap-4 text-white/20">
                    <Cloud size={16} />
                    <span className="text-[10px] font-bold uppercase tracking-widest">AList 分布式存储</span>
                  </div>
