...
import { User, LogOut, Menu, X, ArrowRight, Sun, Moon, Package, Play, Cpu, Info, ExternalLink } from 'lucide-react';
            ...
            return (
              <Link
                ...
                className={cn(
                  ...
                  active ? 'text-[#00ff88]' : 'text-zinc-400 hover:text-white'
                )}
              >
                <Icon size={14} />
                {item.label}
                {active && (
                  <motion.div
                    layoutId="nav-glow"
                    className="absolute -bottom-1 left-0 right-0 h-0.5 bg-[#00ff88] rounded-full shadow-[0_0_15px_rgba(0,255,136,0.8)]"
                  />
                )}
              </Link>
            );
              ...
              {navItems.map((item: any) => (
                <Link
                  ...
                  className={cn(
                    ...
                    isLinkActive(item) ? 'text-[#00ff88]' : 'text-zinc-400 hover:text-white'
                  )}
                >
                  {item.label}
                </Link>
