...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { url: downloadUrl, error: storageError } = await storage.resolveDownloadUrl(
      asset.downloadUrl,
    ...
    if (!downloadUrl || !downloadUrl.startsWith('http')) {
      ...
      console.error(
        `[Download Proxy] Failed to resolve a valid download URL for asset ${id}. ${errorDetail}`
      );
      return new NextResponse(
        `Failed to resolve a valid download URL. Details: ${errorDetail}. Please check your storage configuration in Admin Settings.`,
        { status: 400 }
      );
    }
    ...
    if (
      ...
      config.alistToken &&
      config.alistUrl &&
      downloadUrl.includes(new URL(config.alistUrl).hostname)
    ) {
    ...
    if (!response.ok) {
      ...
      console.error(
        `[Download Proxy] Failed to fetch from storage. Status: ${response.status}, URL: ${downloadUrl}, Body: ${errorText}`
      );
      return new NextResponse(`Failed to fetch file from storage: ${response.statusText}`, {
    ...
    if (remoteDisposition) {
      const match =
        remoteDisposition.match(/filename\*=UTF-8''([^";]+)/i) ||
    ...
    console.log(
      `[Download Proxy] Starting stream for ${finalFilename} (${contentLength || 'unknown size'})`
    );
