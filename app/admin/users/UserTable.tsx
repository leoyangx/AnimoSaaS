      ...
      <table className="w-full text-left border-collapse">
        <thead>
          <tr className="border-b border-white/10 bg-white/5">
            <th className="px-6 py-4 text-xs font-mono uppercase tracking-widest text-zinc-500">学员信息</th>
            <th className="px-6 py-4 text-xs font-mono uppercase tracking-widest text-zinc-500">IP/城市</th>
            <th className="px-6 py-4 text-xs font-mono uppercase tracking-widest text-zinc-500">注册时间</th>
            <th className="px-6 py-4 text-xs font-mono uppercase tracking-widest text-zinc-500">最后登录</th>
                ...
                <td className="px-6 py-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex flex-col">
                      ...
                      <span className="text-[10px] text-zinc-500 uppercase tracking-widest">Student ID: {user.id.substring(0, 8)}</span>
                    </div>
                  </div>
                </td>
                <td className="px-6 py-4">
                ...
                <td className="px-6 py-4">
                  {user.disabled ? (
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[10px] font-bold text-red-500 uppercase tracking-wider">已禁用</span>
                    </div>
                  ) : (
                    <div className="flex items-center gap-2">
                      <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]" />
                      <span className="text-[10px] font-bold text-emerald-500 uppercase tracking-wider">正常</span>
                    </div>
                  )}
                </td>
                <td className="px-6 py-4">
                  <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    >
                      {loadingId === user.id ? <Loader2 className="animate-spin" size={16} /> : (user.disabled ? <Shield size={16} /> : <ShieldOff size={16} />)}
                    </button>
                    <button 
                      disabled={loadingId === user.id}
                      onClick={() => {
            ...
            >
              ...
              <form onSubmit={addUser} className="space-y-4">
                <div className="space-y-2">
                  <label className="text-xs font-mono uppercase tracking-widest text-zinc-500">学员邮箱</label>
                  <input 
                    required
                    type="email" 
                ...
                <div className="space-y-2">
                  <label className="text-xs font-mono uppercase tracking-widest text-zinc-500">初始密码</label>
                  <input 
                    required
                    type="password" 
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-6">
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <button onClick={() => setIsPasswordModalOpen(false)} className="text-zinc-500 hover:text-white">
                  <X size={20} />
                </button>
              </div>
              <p className="text-sm text-zinc-400">正在为 <span className="text-white font-mono">{selectedUser?.email}</span> 修改密码</p>
              <form onSubmit={changePassword} className="space-y-4">
                <div className="space-y-2">
                  <label className="text-xs font-mono uppercase tracking-widest text-zinc-500">新密码</label>
                  <input 
                    required
                    type="password" 
                    value={newPassword}
