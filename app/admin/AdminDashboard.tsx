...
export default function AdminDashboard({ stats, users, codes, config, chartData: initialChartData, adminLogs = [] }: any) {
  ...
  return (
    <div className="space-y-10 pb-20">
      ...
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h1 className="text-3xl font-display font-black tracking-tight text-gradient-white">管理概览</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          { label: '总学员', value: stats.totalUsers, icon: Users, color: 'text-blue-400', bg: 'bg-blue-400/10', growth: '+12%' },
          { label: '总素材', value: stats.totalAssets, icon: Package, color: 'text-purple-400', bg: 'bg-purple-400/10', growth: '+5' },
          { label: '累计下载', value: stats.totalDownloads, icon: Download, color: 'text-emerald-400', bg: 'bg-emerald-400/10', growth: '+28%' },
          { label: '待用邀请码', value: stats.unusedCodes, icon: Key, color: 'text-orange-400', bg: 'bg-orange-400/10', growth: '-2' },
        ].map((stat, i) => (
          <motion.div
            key={stat.label}
          ...
          >
            <div className="absolute top-0 right-0 p-4">
              <span className={cn(
                ...
                stat.growth.startsWith('+') ? "bg-emerald-500/10 text-emerald-400" : "bg-red-500/10 text-red-400"
              )}>
                {stat.growth}
              </span>
            </div>
            ...
            <div className="text-3xl font-display font-black mb-1 group-hover:text-brand-primary transition-colors">{stat.value}</div>
        ...
        <div className="lg:col-span-3 glass-card p-8">
          <div className="flex items-center justify-between mb-8">
            ...
            <div className="flex gap-2">
              <button className="px-3 py-1.5 rounded-lg bg-brand-primary text-black text-[10px] font-bold uppercase tracking-wider">下载量</button>
              <button className="px-3 py-1.5 rounded-lg bg-white/5 text-zinc-400 text-[10px] font-bold uppercase tracking-wider hover:bg-white/10">注册量</button>
            </div>
          </div>
        ...
        <div className="space-y-6">
            ...
            <div className="space-y-4">
              {[
                ...
                <div key={item.label} className="flex items-center justify-between">
                  ...
                  <span className={cn("text-[10px] font-bold uppercase tracking-wider", item.color)}>{item.status}</span>
                </div>
              ))}
            </div>
          ...
          <div className="glass-card p-6">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              >
                <Plus size={16} className="mx-auto mb-2 text-zinc-400 group-hover:text-brand-primary transition-colors" />
                <span className="text-[10px] font-bold text-zinc-500 group-hover:text-white">发布素材</span>
              </button>
              <button 
                onClick={() => router.push('/admin/codes')}
                className="p-3 rounded-xl bg-white/5 border border-white/10 hover:bg-white/10 transition-all text-center group"
              >
                <Key size={16} className="mx-auto mb-2 text-zinc-400 group-hover:text-brand-primary transition-colors" />
                <span className="text-[10px] font-bold text-zinc-500 group-hover:text-white">生成码</span>
              </button>
              <button 
                onClick={() => router.push('/admin/settings')}
                className="p-3 rounded-xl bg-white/5 border border-white/10 hover:bg-white/10 transition-all text-center group"
              >
                <Settings size={16} className="mx-auto mb-2 text-zinc-400 group-hover:text-brand-primary transition-colors" />
                <span className="text-[10px] font-bold text-zinc-500 group-hover:text-white">系统设置</span>
              </button>
              <button 
                onClick={() => window.open('/', '_blank')}
                className="p-3 rounded-xl bg-white/5 border border-white/10 hover:bg-white/10 transition-all text-center group"
              >
                <Home size={16} className="mx-auto mb-2 text-zinc-400 group-hover:text-brand-primary transition-colors" />
                <span className="text-[10px] font-bold text-zinc-500 group-hover:text-white">预览前台</span>
              </button>
            </div>
          </div>
        </div>
            ...
            {users.slice(0, 5).map((user: any) => (
              <div key={user.id} className="flex items-center justify-between group">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <div className="text-sm font-bold text-white/90">{user.email.split('@')[0]}</div>
                    <div className="text-[10px] text-zinc-500 font-mono">{formatDate(user.createdAt)}</div>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  <span className="text-[10px] px-2 py-0.5 rounded-full bg-blue-500/10 text-blue-400 border border-blue-500/20 font-bold uppercase">学员</span>
                  <button 
                    onClick={() => router.push('/admin/users')}
        ...
        <div className="lg:col-span-2 glass-card p-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {codes.slice(0, 6).map((code: any) => (
              <div key={code.code} className="flex items-center justify-between p-4 rounded-2xl bg-white/[0.02] border border-white/5 hover:border-white/10 transition-all group">
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex flex-col">
                    ...
                    <span className="text-[10px] text-zinc-500 uppercase tracking-tighter">
                      {code.status === 'unused' ? '待使用' : `使用者: ${code.usedBy?.substring(0, 8)}...`}
                    </span>
                  </div>
                </div>
                {code.status === 'unused' ? (
        ...
        <div className="glass-card p-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="p-5 rounded-2xl bg-white/[0.02] border border-white/5">
              <div className="text-[10px] font-bold text-zinc-600 uppercase tracking-widest mb-2">驱动程序</div>
            ...
            <div className="p-5 rounded-2xl bg-white/[0.02] border border-white/5">
              <div className="text-[10px] font-bold text-zinc-600 uppercase tracking-widest mb-2">主节点</div>
              <div className="font-mono font-bold text-sm text-white/90 truncate">
                {config.alistUrl || config.juheUrl || 'Local'}
              </div>
            </div>
            <div className="p-5 rounded-2xl bg-white/[0.02] border border-white/5">
              <div className="text-[10px] font-bold text-zinc-600 uppercase tracking-widest mb-2">API 响应</div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-white/5">
                <th className="pb-4 text-[10px] font-bold uppercase tracking-widest text-zinc-500">时间</th>
                <th className="pb-4 text-[10px] font-bold uppercase tracking-widest text-zinc-500">管理员</th>
                <th className="pb-4 text-[10px] font-bold uppercase tracking-widest text-zinc-500">操作</th>
                <th className="pb-4 text-[10px] font-bold uppercase tracking-widest text-zinc-500">详情</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-white/[0.02]">
              {adminLogs.length === 0 ? (
                <tr>
                  <td colSpan={4} className="py-8 text-center text-zinc-500 text-sm">暂无操作记录</td>
                </tr>
              ) : (
                adminLogs.map((log: any) => (
                  <tr key={log.id} className="group hover:bg-white/[0.01] transition-colors">
                    <td className="py-4 text-xs font-mono text-zinc-500">{formatDate(log.createdAt)}</td>
