...
import { Menu, Plus, Trash2, GripVertical, Save, ExternalLink, Package, Play, Cpu, Info } from 'lucide-react';
    ...
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-zinc-500 text-sm mt-1">配置站点顶部及移动端导航菜单，支持业务模块映射</p>
        </div>
          ...
          <div key={item.id} className="glass-card p-6 flex items-center gap-6 group">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6 flex-1">
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase tracking-widest text-zinc-500">显示名称</label>
                <input
                  type="text"
                  value={item.label}
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase tracking-widest text-zinc-500">链接类型</label>
                <select
                  value={item.type}
                ...
                >
                  <option value="MODULE">业务模块映射</option>
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase tracking-widest text-zinc-500">目标值</label>
                {item.type === 'MODULE' ? (
                  <select
                    value={item.value}
                  ...
                  <input
                    ...
                    value={item.value}
                    onChange={(e) => updateItem(item.id, { value: e.target.value })}
                    placeholder={item.type === 'CATEGORY' ? '输入分类名称' : 'https://...'}
                    className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2 text-sm text-white focus:border-[#00ff88] outline-none transition-colors"
                  />
                )}
              </div>
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase tracking-widest text-zinc-500">图标</label>
                <select
                  value={item.icon}
