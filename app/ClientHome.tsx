        ...
        <div className="flex flex-col gap-4 pb-4 border-b border-white/5">
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
            <div className="flex flex-col md:flex-row md:items-center gap-6 md:gap-12">
              ...
              <div className="flex flex-col gap-1">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-[11px] font-black uppercase tracking-widest text-brand-accent/60">
                    {displayCategoryName}
                  </span>
                </div>
            ...
            <div className="relative w-full max-w-sm group">
              <Search
                className="absolute left-4 top-1/2 -translate-y-1/2 text-text-secondary group-focus-within:text-brand-accent transition-colors"
                size={18}
              />
              <input
                type="text"
        ...
        {paginatedAssets.length > 0 ? (
          <div className="space-y-12">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-8">
              {paginatedAssets.map((asset: Asset, index: number) => (
                ...
                >
                  <AssetCard asset={asset} isLoggedIn={isLoggedIn} aspectRatio={cardAspectRatio} />
                </motion.div>
              ))}
            </div>
                    ...
                    {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => {
                      ...
                      if (page === currentPage - 2 || page === currentPage + 2) {
                        return (
                          <span
                            key={page}
                            className="text-text-secondary font-black px-2 opacity-20"
                          >
                            ...
                          </span>
                        );
          ...
          <div className="h-[500px] rounded-[4rem] border border-white/5 bg-white/[0.01] flex flex-col items-center justify-center gap-8">
            ...
            <div className="text-center space-y-4">
              <h3 className="text-4xl font-black tracking-tighter text-text-secondary opacity-40">
                未找到结果
              </h3>
              <p className="text-text-secondary text-sm uppercase tracking-widest font-bold opacity-60">
                请尝试调整您的搜索或筛选条件
              </p>
            </div>
            <button
