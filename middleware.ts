  ...
  if (isV1Route) {
    ...
    if (!authHeader || !authHeader.startsWith('Bearer ')) {
      return NextResponse.json(
        { success: false, error: '缺少 Authorization 头部，格式：Bearer ak_xxx', timestamp: new Date().toISOString() },
        { status: 401 }
      );
    }
    ...
    if (!apiKey) {
      return NextResponse.json(
        { success: false, error: 'API Key 无效、已禁用或已过期', timestamp: new Date().toISOString() },
        { status: 401 }
      );
    }

    // 检查租户状态
    const tenant = await getTenantBySlug('').catch(() => null);
    // 直接用 tenantId 查租户状态
